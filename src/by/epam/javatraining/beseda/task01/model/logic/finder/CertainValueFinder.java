...
public class CertainValueFinder {
     ...
     * @param implementation Object, encapsulating the certain value,
     * implementing the CertainValue interface, can be in the next predefined
     * classes:
     * <ul>
    ...
    public static List<Publication> find(PublicationContainer books,
            CertainValue implementation) throws WrongIndexException {
        ArrayList<Publication> list = new ArrayList<>();
        if (books != null && implementation != null) {
            for (int i = 0; i < books.publicationsNumber(); i++) {
                if (implementation.match(books, i)) {
